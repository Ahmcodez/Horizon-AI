  ...
  return (
    ...
    >
      ...
      <UpgradeGate feature="State tax comparison">
        ...
        <p className="text-xs text-lp-slate leading-relaxed mt-8">
...
function StatePicker({
  ...
  return (
    <label className="block">
      <span className="text-sm font-normal text-lp-slate block mb-2">{label}</span>
      <select
        value={value}
...
function StateCard({ info }: { info: StateTaxInfo }) {
  return (
    <div
      ...
      className={`hover-glow-lp rounded-[15px] p-7 border ${
        info.taxesSocialSecurity
          ? 'bg-lp-chalk border-lp-bad/40'
          : 'bg-lp-chalk border-lp-line-strong'
      }`}
    >
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-normal text-lp-graphite">{info.name}</h2>
        <span
          className={`text-[11px] font-mono font-normal uppercase px-2.5 py-1 rounded-[5px] border ${
            info.taxesSocialSecurity ? 'border-lp-bad text-lp-bad' : 'border-lp-good text-lp-good'
          }`}
        >
          {info.taxesSocialSecurity ? 'Taxes SS' : 'No SS tax'}
        </span>
      </div>
      <p className="text-sm text-lp-slate leading-relaxed">{info.notes}</p>
    </div>
  )
}
