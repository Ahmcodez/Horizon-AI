...
export default function HouseholdPanel({ primaryPia, primaryBirthYear }: Props) {
  ...
  return (
    <div className="bg-lp-chalk border border-lp-line-strong rounded-[15px] p-8" style={{ fontFamily: 'var(--font-jakarta)' }}>
      <h2 className="text-xl font-normal mb-1 text-lp-graphite">Household &amp; work situation</h2>
      <p className="text-sm text-lp-slate mb-6">
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-[9999px] text-sm transition-colors duration-500 border ${
              tab === t.id
                ? 'bg-lp-chalk-dim text-lp-graphite border-[var(--color-lp-cyan)]'
                : 'bg-transparent text-lp-slate border-lp-line-strong hover:text-lp-graphite'
            }`}
...
function SpousalTab({ primaryPia, primaryBirthYear }: Props) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-5">
        ...
        <div>
          <span className="text-sm text-lp-graphite block mb-2">
            Spouse claims at age <span className="text-lp-graphite">{spouseClaimAge}</span>
          </span>
          <input
            type="range"
            min={62}
      ...
      <div className="bg-lp-chalk-dim border border-lp-line-strong rounded-[15px] p-6">
        <div className="text-xs uppercase tracking-[0.02em] text-lp-slate mb-3">Result</div>
        <div className="text-3xl font-normal mb-1 text-lp-graphite">
          ${finalBenefit.toLocaleString(undefined, { maximumFractionDigits: 0 })}
          <span className="text-base text-lp-slate font-normal">/mo</span>
        </div>
        <div className="text-sm text-lp-slate mb-4">
          Paid from {source === 'spousal' ? "the spousal benefit (based on your record)" : "the spouse's own work record"}
        </div>
        <div className="space-y-2 text-sm border-t border-lp-line-strong pt-4">
          <div className="flex justify-between">
            <span className="text-lp-slate">Spousal benefit (up to 50% of your PIA)</span>
            <span className="text-lp-graphite">${spousalFromPrimary.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-lp-slate">Spouse's own benefit</span>
            <span className="text-lp-graphite">${spouseOwnBenefit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </div>
        </div>
        <p className="text-xs text-lp-slate mt-4 leading-relaxed">
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-5">
        ...
        <div>
          <span className="text-sm text-lp-graphite block mb-2">
            You claim survivor benefit at age <span className="text-lp-graphite">{survivorClaimAge}</span>
          </span>
          <input
            type="range"
            min={60}
      ...
      <div className="bg-lp-chalk-dim border border-lp-line-strong rounded-[15px] p-6">
        ...
        <div className="text-sm text-lp-slate">
...
function PensionTab() {
  ...
  return (
    <div className="max-w-xl">
      <p className="text-sm text-lp-graphite mb-5">
      ...
      <div className="flex gap-3 mb-6">
        <button
          ...
          className={`px-5 py-2.5 rounded-[9999px] text-sm border transition-colors duration-500 ${
            hasPension === true ? 'bg-lp-chalk-dim text-lp-graphite border-[var(--color-lp-cyan)]' : 'bg-transparent border-lp-line-strong text-lp-slate'
          }`}
          style={{ transitionTimingFunction: 'cubic-bezier(0.52,0.01,0,1)' }}
        >
          Yes
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-[9999px] text-sm border transition-colors duration-500 ${
            hasPension === false ? 'bg-lp-chalk-dim text-lp-graphite border-[var(--color-lp-cyan)]' : 'bg-transparent border-lp-line-strong text-lp-slate'
          }`}
          style={{ transitionTimingFunction: 'cubic-bezier(0.52,0.01,0,1)' }}
        >
          No
      ...
      {status && (
        <div className="rounded-[15px] p-6 bg-lp-chalk border border-lp-line-strong">
          <div className="text-xs uppercase tracking-[0.02em] text-lp-slate mb-2">
            {hasPension ? 'Good news' : 'Not applicable'}
          </div>
          <p className="text-sm text-lp-graphite leading-relaxed">{status.message}</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-5">
        <div>
          <span className="text-sm text-lp-graphite block mb-2">
            You claim at age <span className="text-lp-graphite">{claimAge}</span>
          </span>
          <input
            type="range"
            min={62}
        ...
        />
        {!beforeFra && (
          <p className="text-xs text-lp-slate bg-lp-chalk border border-lp-line-strong rounded-[5px] p-3">
      ...
      <div className="bg-lp-chalk-dim border border-lp-line-strong rounded-[15px] p-6">
        ...
        <div className="text-3xl font-normal mb-1 text-lp-graphite">
          ...
          <span className="text-base text-lp-slate font-normal">/yr net</span>
        </div>
        <div className="space-y-2 text-sm border-t border-lp-line-strong pt-4 mt-4">
          <div className="flex justify-between">
            <span className="text-lp-slate">Gross annual benefit</span>
            <span className="text-lp-graphite">${annualBenefit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-lp-slate">Withheld for earnings over the limit</span>
            <span className="text-lp-bad">
...
function FieldNumber({
  ...
  return (
    <label className="block">
      <span className="text-sm text-lp-graphite block mb-2">{label}</span>
      <div className="relative">
        {prefix && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-lp-slate">{prefix}</span>}
        <input
          type="number"
          value={value}
          min={min}
          ...
          className={`w-full bg-lp-chalk border border-lp-line-strong rounded-[5px] py-3 text-base focus:border-[var(--color-lp-cyan)] outline-none transition-colors text-lp-graphite ${
