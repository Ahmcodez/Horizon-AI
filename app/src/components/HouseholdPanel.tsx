...
export default function HouseholdPanel({ primaryPia, primaryBirthYear }: Props) {
  ...
  return (
    <div className="bg-chalk border border-graphite/10 rounded-3xl p-8 shadow-sm">
      <h2 className="font-display text-xl font-medium mb-1">Household &amp; work situation</h2>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium font-mono transition-colors border ${
              tab === t.id
                ...
                : 'bg-chalk-dim text-slate border-graphite/10 hover:text-graphite'
            }`}
          >
            {t.label}
          </button>
...
function SpousalTab({ primaryPia, primaryBirthYear }: Props) {
  ...
  // Social Security pays the higher of the two - own benefit or spousal benefit
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-5">
        ...
        <div>
          <span className="text-sm font-medium text-graphite block mb-2">
            Spouse claims at age <span className="font-mono text-amber-deep">{spouseClaimAge}</span>
          </span>
          <input
            type="range"
            min={62}
      ...
      <div className="bg-chalk-dim rounded-2xl p-6">
        <div className="text-xs font-mono uppercase tracking-wide text-slate mb-3">Result</div>
        <div className="font-mono text-3xl font-semibold mb-1">
          ${finalBenefit.toLocaleString(undefined, { maximumFractionDigits: 0 })}
          <span className="text-base font-sans text-slate font-normal">/mo</span>
        </div>
        ...
        <div className="space-y-2 text-sm border-t border-graphite/10 pt-4">
          <div className="flex justify-between">
            <span className="text-slate">Spousal benefit (up to 50% of your PIA)</span>
            <span className="font-mono">${spousalFromPrimary.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate">Spouse's own benefit</span>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-5">
        ...
        <div>
          <span className="text-sm font-medium text-graphite block mb-2">
            You claim survivor benefit at age <span className="font-mono text-amber-deep">{survivorClaimAge}</span>
          </span>
          <input
            type="range"
            min={60}
...
function PensionTab() {
  ...
  return (
    <div className="max-w-xl">
      ...
      <div className="flex gap-3 mb-6">
        <button
          ...
          className={`px-5 py-2.5 rounded-full text-sm font-semibold border transition-colors ${
            hasPension === true ? 'bg-graphite text-chalk border-graphite' : 'bg-chalk-dim border-graphite/10 text-slate'
          }`}
        >
          Yes
        </button>
        <button
          ...
          className={`px-5 py-2.5 rounded-full text-sm font-semibold border transition-colors ${
            hasPension === false ? 'bg-graphite text-chalk border-graphite' : 'bg-chalk-dim border-graphite/10 text-slate'
          }`}
        >
          No
        </button>
      ...
      {status && (
        <div className={`rounded-2xl p-6 ${hasPension ? 'bg-amber/10 border border-amber-deep/20' : 'bg-chalk-dim'}`}>
          <div className="text-xs font-mono uppercase tracking-wide text-amber-deep mb-2">
            {hasPension ? 'Good news' : 'Not applicable'}
          </div>
          <p className="text-sm text-graphite leading-relaxed">{status.message}</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-5">
        <div>
          <span className="text-sm font-medium text-graphite block mb-2">
            You claim at age <span className="font-mono text-amber-deep">{claimAge}</span>
          </span>
          <input
            type="range"
            min={62}
        ...
        />
        {!beforeFra && (
          <p className="text-xs text-amber-deep bg-amber/10 rounded-lg p-3">
      ...
      <div className="bg-chalk-dim rounded-2xl p-6">
        ...
        <div className="font-mono text-3xl font-semibold mb-1">
          ...
          <span className="text-base font-sans text-slate font-normal">/yr net</span>
        </div>
        <div className="space-y-2 text-sm border-t border-graphite/10 pt-4 mt-4">
          <div className="flex justify-between">
            <span className="text-slate">Gross annual benefit</span>
            <span className="font-mono">${annualBenefit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate">Withheld for earnings over the limit</span>
  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        {prefix && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate font-mono">{prefix}</span>}
        <input
          type="number"
          value={value}
          min={min}
          ...
          className={`w-full bg-chalk border border-graphite/15 rounded-xl py-3 font-mono text-base focus:border-amber-deep outline-none transition-colors ${
